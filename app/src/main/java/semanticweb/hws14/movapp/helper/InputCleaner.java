...
public class InputCleaner {
    ...
    public static String cleanDirectorName(String directorName) {
        directorName = directorName.trim();
        final StringBuilder result = new StringBuilder(directorName.length());
        String[] words = directorName.split("\\s");
        for(int i=0; i<words.length; i++) {
            if(i>0) {
                result.append(" ");
            }
